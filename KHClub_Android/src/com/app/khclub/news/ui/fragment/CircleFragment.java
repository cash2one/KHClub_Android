...
public class CircleFragment extends BaseFragment {
	...
	private void initListViewSet() {
		...
		// 设置内容
		circleAdapter = new HelloHaAdapter<CircleModel>(getActivity(), R.layout.fragment_circle_attention) {
			@Override
			protected void convert(HelloHaBaseAdapterHelper helper, final CircleModel item) {
				// 圈子标题
				...
				if (null != images && images.length > 0) {
					ImageLoader.getInstance().displayImage(KHConst.ATTACHMENT_ADDR + images[0], headImageView,
							headImageOptions);
				} else {
		...
		// 设置底部自动刷新
		circleListView.setOnLastItemVisibleListener(new OnLastItemVisibleListener() {
		...
		HttpManager.get(path, new JsonRequestCallBack<String>(new LoadDataHandler<String>() {
			...
			public void onSuccess(JSONObject jsonResponse, String flag) {
				...
				if (status == KHConst.STATUS_SUCCESS) {
					...
					list = JSON.parseArray(jsonArrayStr, CircleModel.class);
					// 如果是下拉刷新
					if (isPullDowm) {
...
//private class MyItem implements MultiItemTypeSupport{
//
//	@Override
