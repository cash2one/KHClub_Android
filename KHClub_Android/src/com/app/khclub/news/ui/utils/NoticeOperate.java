...
public class NoticeOperate {
	...
	public NoticeOperate(Context context) {
		...
		HttpManager.post(KHConst.DELETE_NOTICE, params, new JsonRequestCallBack<String>(new LoadDataHandler<String>() {
			...
			public void onSuccess(JSONObject jsonResponse, String flag) {
				...
				if (status == KHConst.STATUS_SUCCESS) {
					callInterface.onFinish(OP_Type_Delete_News, true, null);
				}

				if (status == KHConst.STATUS_FAIL) {
					callInterface.onFinish(OP_Type_Delete_News, false, null);
	...
	public void publishComment(final UserModel user, final String... parameter) {
		...
		HttpManager.post(KHConst.SEND_NOTICE_COMMENT, params,
					...
					public void onSuccess(JSONObject jsonResponse, String flag) {
						...
						if (status == KHConst.STATUS_SUCCESS) {
							...
							if (parameter.length >= 4) {
								...
								temMode.setTarget_name("");
							}

							callInterface.onFinish(OP_Type_Add_Comment, true, temMode);
						}

						if (status == KHConst.STATUS_FAIL) {
							callInterface.onFinish(OP_Type_Add_Comment, false, null);
							// if (jsonResponse.getInteger(KHConst.HTTP_RESULT)
							// == 1) {
							// // 没关注圈子
							// ToastUtil.show(mContext,
							// mContext.getString(R.string.circle_no_follow));
							// } else {
							ToastUtil.show(mContext, mContext.getString(R.string.network_unavailable));
							// }
						}
	...
	public void deleteComment(String CID, String noticeID) {
		...
		HttpManager.post(KHConst.DELETE_NOTICE_COMMENT, params, new JsonRequestCallBack<String>(new LoadDataHandler<String>() {
			...
			public void onSuccess(JSONObject jsonResponse, String flag) {
				...
				int status = jsonResponse.getInteger(KHConst.HTTP_STATUS);
				if (status == KHConst.STATUS_SUCCESS) {
					callInterface.onFinish(OP_Type_Delete_Comment, true, null);
				}

				if (status == KHConst.STATUS_FAIL) {
					callInterface.onFinish(OP_Type_Delete_Comment, false, null);
					ToastUtil.show(mContext, mContext.getString(R.string.network_unavailable));
				}
			}
	 ...
	 *            是不是点赞操作
	 */
	public void uploadLikeOperate(String noticeId, boolean isLike) {
		if (!isUploadData) {
			isUploadData = true;
			if (isLike) {
			...
			// Log.i("wx", KHConst.NOTICE_LIKE_OR_CANCEL);
			HttpManager.post(KHConst.NOTICE_LIKE_OR_CANCEL, params,
					new JsonRequestCallBack<String>(new LoadDataHandler<String>() {
