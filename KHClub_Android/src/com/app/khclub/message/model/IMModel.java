package com.app.khclub.message.model;
...
public class IMModel {
	...
	public void save() {
		...
		if (null == findByGroupId(targetId)) {
			...
			String sql = "INSERT INTO kh_group values (null,'"+targetId+"','"+title+"','"+type+
	...
	public void deleteData() {
		String sql = "DELETE FROM kh_group";
		DBManager.getInstance().excute(sql);
	}
	//更新数据
	public void update() {
		...
		String sql = "UPDATE kh_group SET groupTitle = '"+title+"', type="+type+", " +
	...
	//删除数据
	public void remove() {
		String sql = "DELETE FROM kh_group WHERE groupId='"+targetId+"' AND owner='"+owner+"'";
		Log.i("NewFriendsActivity", sql);
		DBManager.getInstance().excute(sql);
	}
//	//查询全部数据
//	public List<IMModel> findAll() {
//		String sql = "SELECT * FROM kh_group WHERE type=1 " +
...
//	}
	...
	public static List<IMModel> findHasAddAll() {
		String sql = "SELECT * FROM kh_group WHERE type=1 AND currentState=1" +
			     " AND owner='"+UserManager.getInstance().getUser().getUid()+"' ORDER BY id DESC";
		return findBySql(sql);
	}
	//从数据库中查出新的朋友群组 最近的三条没有被添加的人
	public static List<IMModel> findThreeNewFriends() {
		String sql = "SELECT * FROM kh_group WHERE type=1 and isNew=1 and isRead=0" +
	...
	public static List<IMModel> findAllNewFriends() {
		String sql = "SELECT * FROM kh_group WHERE type=1 and isNew=1" +
	...
	public static void hasRead() {
		String sql = "UPDATE kh_group SET isRead=1 WHERE owner='"+UserManager.getInstance().getUser().getUid()+"'";
		DBManager.getInstance().excute(sql);
	}
	...
	public static IMModel findByGroupId(String targetId) {
		...
		String sql = "SELECT * FROM kh_group WHERE owner='"+UserManager.getInstance().getUser().getUid()+"' " +
